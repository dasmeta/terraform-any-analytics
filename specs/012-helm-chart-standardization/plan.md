...
- The centrally managed repository baseline currently requires legacy `terraform-validate`, `terraform-validate (/)`, `terraform-tfsec`, and `publish` contexts that it does not reliably generate for this multi-module repository. Until the generator is repaired under DMVP-10370, this branch emits compatibility contexts backed by real module validation, tfsec, and semantic-release jobs. Branch protection is not changed manually.
